...
def immediate_transaction() -> Generator[Session, None, None]:
    """Run a serialized writer transaction.

    On SQLite, uses ``BEGIN IMMEDIATE`` to reserve a writer lock (SQLite has no
    ``FOR UPDATE SKIP LOCKED``).  On PostgreSQL, uses a regular transaction;
    individual queries rely on row-level locking with ``FOR UPDATE SKIP LOCKED``.
    """
